...
def plotJumpHeightsA(data):
    ...
    for r in ratios:
        ...
        print(area)
        print(r)
        ax.fill_between(area[:,0],area[:,1],area[:,2], color = plt.cm.coolwarm(r/170.0), alpha=0.5)
    ...
    for r in ratios:
       ...
       # print('in here')
        for l in lengths:
            #if l > 12:
                #continue
            indeces = np.where(((data[:,0:2] == (r,l)).all(axis=1)))
            if l < 1:
                continue
            heights = height[indeces]
            heights[heights<0] = 0
        ...
        #print(area)
        #print(r)
        plt.plot(dataSet[:,0],dataSet[:,1], marker = markers[m], color = plt.cm.plasma_r(r/400.0), alpha=0.95)
        m+=1
...
# print(data2[:,2])
# =============================================================================
data2 = np.genfromtxt('C:/Users/Jacob/Documents/Senior/IDEAlab/simulation/pullups/build/unity_results.csv', delimiter=',')
...
def plotJumpHeights1(data):
    ...
    for r in ratios:
        ...
        #print(area)
        #print(r)
        plt.plot(dataSet[:,0],dataSet[:,1], marker = 'x', color = plt.cm.coolwarm(r/120.0), alpha=0.95)
