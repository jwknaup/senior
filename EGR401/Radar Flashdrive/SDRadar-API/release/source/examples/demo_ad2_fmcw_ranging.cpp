// = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = = =
// Title:    SDR_API Demo - Display Real-Time Range Information (AD2)
//
// Author:   Ancortek Inc.
// Contact:  info@ancortek.com
//
// Description:
//   Demo. This demonstration instantiates an SDRadar2 class and configures
...
int main (int argc, char* argv[])
{
    ...
    size_t char_buffer_size = samples_per_sweep * 2 * 2 * 2 + SDR_PEAK_SEARCH_HEAD + SDR_SKIP_HEAD;
    ...
    double range_factor = 3.0e8 / 2.0 / 1500.0e6;
    ...
    while (true)
    {
        ...
        {
            // Uncomment one of the following to select RX1 or RX2
            sweep_raw[i] = std::complex<double>(buffer_uint16[4 * i + 1], buffer_uint16[4 * i] & (unsigned short) 0x7FFF); // Channel RX1
            //sweep_raw[i] = std::complex<double>(buffer_uint16[4 * i + 3], buffer_uint16[4 * i + 2] & (unsigned short) 0x7FFF); // Channel RX2
        }
